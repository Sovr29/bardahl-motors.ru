<?php
class ModelCheckout2Order extends Model {
	public function addOrder($data) {
		...
		if (isset($data['vouchers'])) {
			foreach ($data['vouchers'] as $voucher) {
				...
				$voucher_id = $this->model_checkout2_voucher->addVoucher($order_id, $voucher);
	...
	public function editOrder($order_id, $data) {
		...
		$this->model_checkout2_voucher->disableVoucher($order_id);
        ...
        public function updateUnitPayOrderStatus($order_id, $status_id) {
		$this->event->trigger('pre.order.edit', $order_id);

		// Void the order first
		$this->addOrderHistory($order_id, $status_id, 'Изменен статус заказа после оплаты через UnitPay');
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$order_product_query = $this->db->query("SELECT op.*, p.sku FROM " . DB_PREFIX . "order_product op inner join " . DB_PREFIX . "product p on p.product_id = op.product_id WHERE op.order_id = '" . (int)$order_id . "'");
				...
				$data['text_instruction'] = $language->get('text_new_comment');
				$data['text_order_id'] = $language->get('text_new_order_id');
				$data['text_date_added'] = $language->get('text_new_date_added');
				$data['text_fio'] = $language->get('text_new_fio');
				...
				$data['shipping_fio'] = $order_info['shipping_lastname'].' '.$order_info['shipping_firstname'];
			...
			if (in_array($order_info['order_status_id'], $this->config->get('config_complete_status'))) {
				...
				$this->model_checkout2_voucher->confirm($order_id);
			}
		}
