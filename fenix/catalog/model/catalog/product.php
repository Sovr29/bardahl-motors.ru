...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'admission'=> $query->row['admission'],
				'technical_info'=> $query->row['technical_info'],
				'short_description'=> $query->row['short_description'],
				'europe_quality'   => $query->row['europe_quality'],
				'profitably'       => $query->row['profitably'],
	...
	public function getProducts($data = array()) {
		...
		$sql .= " LEFT JOIN " . DB_PREFIX . "product_type pt ON (p.type_id = pt.type_id)";
		...
		if (!empty($data['filter_manufacturer_id'])) {
			$sql .= " AND p.manufacturer_id = '" . (int)$data['filter_manufacturer_id'] . "'";
                ...
                if(!isset($data['dont_check_tatus']))
                {
                    $sql .= " and ((select count(c.category_id) from " . DB_PREFIX . "category as c inner join " . DB_PREFIX . "product_to_category as pc on pc.category_id = c.category_id where pc.product_id = p.product_id and c.status=1)) > 0";
                }
		...
		$sort_data = array(
			...
			'p.sort_order',
			'p.date_added',
			'pt.sort_order, p.type_id, p.sort_order'
		);
	...
	public function getBestSellerProducts($limit) {
		...
		if (!$product_data) {
			...
			$query = $this->db->query("SELECT op.product_id, SUM(op.quantity) AS total FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) LEFT JOIN `" . DB_PREFIX . "product` p ON (op.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE o.order_status_id > '0' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' and ((select count(c.category_id) from " . DB_PREFIX . "category as c inner join " . DB_PREFIX . "product_to_category as pc on pc.category_id = c.category_id where pc.product_id = op.product_id and c.status=1)) > 0 GROUP BY op.product_id ORDER BY total DESC LIMIT " . (int)$limit);
