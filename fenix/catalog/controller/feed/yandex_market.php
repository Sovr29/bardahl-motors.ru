...
//				$data['cbid'] = 15;
				...
				$data['price'] = number_format($this->currency->convert($this->tax->calculate($product['price'], $product['tax_class_id']), $shop_currency, $offers_currency), $decimal_place, '.', '');
                                if($product['discountedPrice'])
                                {
                                    ...
                                    $data['price'] = number_format($this->currency->convert($this->tax->calculate($product['discountedPrice'], $product['tax_class_id']), $shop_currency, $offers_currency), $decimal_place, '.', '');
                                }
				$data['currencyId'] = $offers_currency;
				$data['categoryId'] = $product['category_id'];
				$data['delivery'] = 'true';
//				$data['local_delivery_cost'] = 100;
				$data['name'] = 'Bardahl ' . $product['name'];
...
//				$data['barcode'] = $product['sku'];
				if ($product['image']) {
					$data['picture'] = str_replace(' ', '%20', $this->model_tool_image->resize($product['image'], 300, 300));
				}
/*
