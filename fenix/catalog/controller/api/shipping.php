		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($this->cart->hasShipping()) {
				...
				if (empty($this->session->data['shipping_methods'])) {
					...
					if (!isset($shipping[0]) || !isset($shipping[1]) || !isset($this->session->data['shipping_methods'][$shipping[1]]['quote'][$shipping[1]])) {
						$json['error'] = $this->language->get('error_method');
					}
				}

				if (!$json) {
					$this->session->data['shipping_method'] = $this->session->data['shipping_methods'][$shipping[1]]['quote'][$shipping[1]];
