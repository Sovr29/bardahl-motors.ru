...
class ControllerInformationNews extends Controller {
	public function index() {
		...
		foreach ($all_news as $news) {
			$data['all_news'][] = array (
				...
				'image'			=> $this->model_tool_image->resize($news['image'], 100, 110),
	...
	public function news() {
		...
		if ($news) {
			...
			$this->document->setName($news['title']);
                        
			$this->load->model('tool/image');
			
			$data['image'] = $this->model_tool_image->resize($news['image'], 1200, 400);
 
                        $this->document->setDescription(html_entity_decode($news['meta_description'], ENT_QUOTES));
                        $this->document->setKeywords(html_entity_decode($news['meta_keyword'], ENT_QUOTES));
			...
			$data['description'] = html_entity_decode($news['description'], ENT_QUOTES);
                        ...
                        $data['fb'] = $news['fb'];
                        $data['vk'] = $news['vk'];
                        $data['insta'] = $news['insta'];
                        $data['date_added'] = $news['date_added'];
